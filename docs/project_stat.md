...
## Feature Enhancement: Mechanical Speed Averaging (2025-12-17)

### Problem Statement

On real hardware testing with laser speedometer, speed measurements were found to be inconsistent. In motors with multiple pole pairs, the electrical speed differs from mechanical speed. Manufacturing variations between pole pairs can cause speed oscillations at each Hall edge, leading to unstable speed control.

### Root Cause

- Hall sensors measure electrical speed (one cycle per pole pair)
- Multiple pole pairs mean multiple electrical cycles per mechanical rotation
- Slight manufacturing differences between poles cause speed variations
- Each Hall edge (60° electrical) can show different speed readings
- Speed controller using instantaneous electrical speed sees rapid oscillations

### Solution Implemented

**Mechanical Speed Averaging Over Pole Pairs:**

1. **Added `MOTOR_POLE_PAIRS` macro** in `motor/foc_define_parameter.h`
   - User-configurable parameter (default: 7 pole pairs)
   - Determines number of electrical cycles per mechanical rotation
   - Documented with examples and tuning guidance

2. **Circular Buffer Implementation** in `motor/hall_sensor.c`
   - Buffer size: `MOTOR_POLE_PAIRS * 6` (6 Hall edges per electrical cycle)
   - Stores Hall edge intervals for one complete mechanical rotation
   - Efficient circular indexing with wrap-around

3. **Averaged Mechanical Speed Calculation**
   - Sums all intervals in circular buffer
   - Calculates average interval per Hall edge
   - Computes mechanical speed: `HALL_SPEED_FACTOR / average_interval`
   - Before buffer fills: uses instantaneous speed / pole pairs

4. **Speed Controller Integration** in `motor/adc.c`
   - Changed speed feedback from `hall_speed` to `hall_mech_speed`
   - Updated field-weakening threshold check
   - Updated ADRC ESO initialization
   - Applied to all three sensor modes (Hall, Sensorless, Hybrid)

### Benefits

- **Stable Speed Control:** Averaging eliminates pole-to-pole variations
- **Reduced Speed Oscillations:** Mechanical speed is inherently smoother
- **Better Disturbance Rejection:** Speed controller sees actual mechanical speed
- **Configurable:** Users can set pole pairs for their specific motor
- **Backward Compatible:** Falls back to electrical speed during buffer fill

### Files Modified

- `motor/foc_define_parameter.h`: Added `MOTOR_POLE_PAIRS` macro with documentation
- `motor/hall_sensor.h`: Added `hall_mech_speed` extern declaration
- `motor/hall_sensor.c`: Implemented circular buffer and averaging algorithm
- `motor/adc.c`: Updated speed feedback to use mechanical speed
- `motor/low_task.c`: Initialize mechanical speed on motor start

### Usage

Users should set `MOTOR_POLE_PAIRS` in `foc_define_parameter.h` according to their motor:
- Count rotor magnets and divide by 2, OR
- Rotate motor slowly and count Hall cycles per mechanical revolution

Example: 14-pole motor (7 magnets per side) = 7 pole pairs

### Technical Notes

- **ISR Safe:** Averaging done in Hall ISR, bounded loop iterations
- **Static Allocation:** No dynamic memory, embedded-safe design
- **Deterministic:** Buffer size known at compile time
- **Fast:** Simple addition and division, no heavy math
- **Startup Behavior:** Uses instantaneous speed until buffer fills (42 Hall edges for 7 pole pairs)

### Testing Recommendations

1. Verify `MOTOR_POLE_PAIRS` matches motor specification
2. Test at various speeds (low, medium, high)
3. Compare speed stability vs. previous electrical speed
4. Monitor speed controller performance during load changes
5. Use laser speedometer or encoder to validate accuracy
